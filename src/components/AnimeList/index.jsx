...
const AnimeList = ({ api }) => {
  return (
          <div className="grid md:grid-cols-4 sm:grid-cols-3 grid-cols-2 gap-4 p-4 rounded-lg ">
        {api.data?.map((anime, index) => {
          return (
            <Link href={`/anime/${anime.mal_id}`} className='cursor-pointer text-primary rounded-lg transition-all' key={index}>
